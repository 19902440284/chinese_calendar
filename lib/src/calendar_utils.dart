...
class CalendarUtils {
  ...
  DateTime? lunar2solar(DateTime dateTime, bool isLeapMonth) {
    ...
    var calObj = DateTime.fromMillisecondsSinceEpoch(
        (offset + d - 31) * 86400000 + stmap.millisecondsSinceEpoch);
  ...
  CalendarInfo getInfo(DateTime solarDate) {
    ...
    var firstNode =
        this.getTerm(solarDate.year, (solarDate.month * 2 - 1)); //返回当月「节」为几日开始
    var secondNode =
        this.getTerm(solarDate.year, (solarDate.month * 2)); //返回当月「节」为几日开始

    // 依据12节气修正干支月
    var gzMonth =
        this.toGanZhi((solarDate.year - 1900) * 12 + solarDate.month + 11);
    if (solarDate.day >= firstNode) {
      gzMonth =
          this.toGanZhi((solarDate.year - 1900) * 12 + solarDate.month + 12);
    }
